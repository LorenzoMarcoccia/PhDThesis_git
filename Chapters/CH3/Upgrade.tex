...
These schemes (summarized in Table~\ref{tab:trig_scheme}) are based on different requirements on the 
four layers of RPC chambers:
\begin{itemize}
...
\begin{figure}[!h]
	\centering
	\includegraphics[width=0.7\textwidth]{Chapters/CH3/figures/csm_pic}
	...
	\label{fig:csm_pic}
\end{figure}
Cluster size is given by the number of strips fired at the same time. It depends on the absolute value of charge integrated over a strip but it also depends on the the position of the true hit.\\
...
\begin{figure}[!h]
	\centering
	\includegraphics[width=1\textwidth]{Chapters/CH3/figures/CS}
...
\subsection{BI RPCs with two-sides \textbf{$\eta − \eta$} readout}
\label{sec:etaeta}

\section{L0 barrel trigger efficiency}
\label{sec:eff}
The performance of the barrel muon trigger was studied for a fixed transverse momentum threshold: $p_{T} > 10$ GeV~\cite{Marcoccia:2693982}.
To study the robustness of the trigger against possible efficiency reductions of the old RPCs in the BM and BO layers, the simulation was performed in the so-called "worst-case-scenario" that introduces inefficiencies depending on the station type and sector and summarized in Table~\ref{tab:WCS}. This scenario include inefficiencies due to a reduction of the high voltage of the BM and BO RPCs such that the expected RPC current is always below the safe operation limit.
...
\begin{table}[htbp]
	...
	\end{center} 
\end{table} 
\noindent The trigger efficiency times acceptance is defined as the fraction of reconstructed muons that are accepted by the trigger, using the simulation that includes the RPC detector efficiency.
Starting from the WCS scenario, several other cases were studied in order to understand what would be the impact of retrofitting some chambers with new electronics.
\newpage
\subsubsection{BM and BO retrofitting}
\label{sec:BMBO_retrofit}
In the first case presented, efficiencies of the "worst-case-scenario" are used except for the following stations, setted to 100\% efficiency:
...
The trigger efficiency times acceptance is also presented in Figure~\ref{fig:allcasesBMBO} that compare the "worst-case scenario" with all the variants of the WCSe, in which some stations are fixed to 100\% efficiency.
\begin{table}[htbp]
	\begin{center}
		\small
...
\begin{figure}[hb]
	\centering
	\subfigure [3/3 chamber]{\includegraphics[width=0.9 \textwidth]{Chapters/CH3/figures/BMBO_firstCase}} 
\end{figure}

\begin{figure}[hb]
	\centering
	\subfigure [3/4 chambers]{\includegraphics[width=0.9 \textwidth]{Chapters/CH3/figures/BIBM_firstCase}} 
	\subfigure [3/4 chambers + BIBO]{\includegraphics[width=0.9 \textwidth]{Chapters/CH3/figures/BIBO_firstCase}} 
...
\subsubsection{Dropping BIR and BIM chambers}
...
\begin{figure}[hb]
	\centering
	\subfigure [3/3 chambers]{\label{fig:allcasesBIRBIM}\includegraphics[width=0.9 \textwidth]{Chapters/CH3/figures/BMBO_secondCase_phi}} 
\end{figure}	

\begin{figure}
	\centering	
	\subfigure [3/4 chambers]{\label{fig:allcasesBIRBIM}\includegraphics[width=0.9 \textwidth]{Chapters/CH3/figures/BIBM_secondCase_phi}} 
	\subfigure [3/4 chambers + BIBO]{\label{fig:allcasesBIRBIM}\includegraphics[width=0.9 \textwidth]{Chapters/CH3/figures/BIBO_secondCase_phi}} 
