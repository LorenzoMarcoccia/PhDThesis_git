...
\item The list of the systematic shapes that are dropped from the fit for each sample and for each region are shown in \Cref{fig:stat:tzc:splusb:crsr:pruning1,fig:stat:tzc:splusb:crsr:pruning2,fig:stat:tzc:splusb:crsr:pruning3,fig:stat:tzc:splusb:crsr:pruning4}.
...
\begin{itemize}
	...
	\item The list of the systematic shapes that are dropped from the fit for each sample and for each region is shown in \Cref{fig:stat:tzc:splusb:crsr:pruning_unb1,fig:stat:tzc:splusb:crsr:pruning_unb2,fig:stat:tzc:splusb:crsr:pruning_unb3,fig:stat:tzc:splusb:crsr:pruning_unb4}.
...
The background estimate is similar to what was found with blinded Signal Regions (\Cref{sec:stat:tzc:splusb:crsr}).\\
...
\begin{table}[]
	\centering
	\tiny
	\input{Chapters/CH8/figures/SPLUSB_CRSR_DL1rc_unblind/Tables/Yields_clean}
...
\begin{table}[]
	\centering
	\tiny
	\input{Chapters/CH8/figures/SPLUSB_CRSR_DL1rc_unblind/Tables/Yields_postFit}
...
The expected upper limit for the \tZc coupling is improved by a factor of 3.3 with respect to the expectation in the previous analysis.
\begin{table}[htbp]
	\centering
	\begin{tabular}{lccc} 
		\toprule
...
\Cref{fig:stat:tzc:splusb:crsr:CLsPlot} shows the observed and expected $\mathrm{CL_{s}}$ as a function of $\mathrm{BR (\Pqt\rightarrow\PZ\Pqc)}$. The observed limit is $\mathrm{BR (\Pqt\rightarrow\PZ\Pqc)}<11.8\times 10^{-5}$, inside the $\pm$1$\sigma$ band of the expected limit.
%Compared to the observed upper limit extracted in the previous analysis, the  observed upper limit extracted in this thesis is improved by a factor of 2, while the expected upper limit is improved by a factor of 2.5
The observed upper limit is improved by a factor of 2.
%, while the expected upper limit is improved by a factor of 2.5, both compared to the observed upper limit extracted by the previous analysis.
