...
\Cref{chapter:charm_tag} described the event selection targeting the FCNC \ttbar decay process using a charm-tagger. To extract limits on the FCNC \tZc coupling, two addition selections can be investigated: FCNC \ttbar decays without the use of a charm-tagger and FCNC single top production.\\
...
For SR3\tZc, the selection presented in \Cref{sec:other_selection}, using \DLrc, is using in the following.
...
selecting events with different jet multiplicities and by selecting or vetoing the presence of c-tagged jet.\\
...
The signal yield in SR3 is effected by the \DLrc  \Pqc-efficiency (20\%), as discussed in~\Cref{sec:object:cjet} but it also has the best background rejection.
