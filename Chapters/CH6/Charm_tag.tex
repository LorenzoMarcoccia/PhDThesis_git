...
\chapter{FCNC \tZc with charm-tagging}
\label{chapter:charm_tag}
This chapter presents an important fraction of my work. It was dedicated to the event selection using the SMT technique, the charm-tagger \DLrc and the comparison between these two techniques.
The design and optimization of the multivariate analysis is part of my work as well, and it will be presented in this chapter.
...
\label{sec:selection}
The channel presented in this section is \FCNCtZc in \ttbar decays, using the Soft Muon Tagging.
...
Even though the selection with exactly one \Pqb-jet is the purest, for the SR3\tZc, events containing zero and two \Pqb-jets are also considered in order to have the largest possible signal acceptance and then work on the separation of signal from background, as described in \Cref{sec:separation}.
\begin{table}[!h]
	\centering
	\footnotesize
	\input{Chapters/CH6/tables/yield_sr3_withSMT.tex}
...
\end{itemize}
According to the categories described above, Table~\ref{tab:sig_comp} shows the composition for the signal sample FCNC $t\bar{t}$(cZ). It can be noted that soft muons mostly come from B-hadrons ($\sim$ 60\%)  and C-hadrons ($\sim$ 40\%) decays.
...
\begin{table}[!h]
	\centering        
	\footnotesize  
	\input{Chapters/CH6/tables/sig_comp.tex}
...
\begin{table}[!h]
	\centering     
	\footnotesize     
	\input{Chapters/CH6/tables/ttZ_comp.tex}
...
\begin{table}[!h]
	\centering    
	\footnotesize      
	\input{Chapters/CH6/tables/VV_comp.tex}
...
SR3\tZc. Those variables are listed in \Cref{tab:D3input}, ordered by the separation value, defined by, as in~\cite{TMVA}:
\begin{equation*}
...
The distributions of input variables in the Signal Region is presented in~\Cref{fig:separation:SR3}.
...
\begin{table}[!htbp]
	...
	\end{tabular}
	\caption{
	Set of variables used in the training of the GBDT in SR3\tZc to built the \Dthree discriminant. Variables are ordered by the separation $\langle s^{2}\rangle$ value. }
	\label{tab:D3input}
\end{table}
...
\begin{itemize}
	...
	\item No requirements on the masses of both the FCNC and the SM top-quark candidates are applied. 
\end{itemize}
This selection is summarized in Table~\ref{tab:sel:sr3_dl1rc} together with the selection using SMT for comparison. Kinematic plots are presented in \Cref{app:SRs:SR3}.
\begin{table}[!h]
	\centering
	\small
...
\begin{table}[!h]
	\centering
	\small
	\input{Chapters/CH6/tables/yield_sr3_withDL1r.tex}
...
To choose the best selection for this analysis, one can compare the values of \ssplusb for each bin of the GBDT discriminant as can bee seen in Table~\ref{tab:yields:sr3_smt_bdt} for the SMT selection, in Table~\ref{tab:yields:sr3_dl1rc_bdt} for \DLrc selection, and Figure~\ref{fig:yields:sr3_bdt}.\\
