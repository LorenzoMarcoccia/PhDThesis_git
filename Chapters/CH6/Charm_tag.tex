...
This chapter presents an important fraction of my work. It is dedicated to the event selection using the SMT technique, the charm-tagger \DLrc and the comparison between these two techniques.
...
\begin{itemize}
	\item    Exactly three leptons (electrons or muons) are required. 
...
Given the topology of the FCNC \tZc decay, the next step is the reconstruction of the two top quarks.
...
\subsection {Signal Region with SMT requirement}
\label{sec:sel:sr3}
...
Given the event yields in Table~\ref{tab:yields:sr3}, a multivariate analysis (MVA) technique is used to have a better separation of the signal from the background and to increase the value of \ssplusb.\\ 
Therefore, the Gradient Boosted Decision Trees (GBDT) method with TMVA software package~\cite{BDTG,TMVA} is exploited in this study. 
...
verify if there is disagreement between the training sample and the test sample and therefore to 
